...
`src/playground.css` owns the fork's shell geometry; `RepairPanel.css` owns the repair rail's inner presentation. Desktop uses a side rail, smaller screens use a dismissible drawer with source/Stop available. Keep source labels readable and touch targets at least 44px. Only fresh starter/recorded demos use `layoutDemo` on phones and tall desktop canvases; never automatically rearrange a saved design. Palette placement must avoid foreground note hit areas as well as nodes.
